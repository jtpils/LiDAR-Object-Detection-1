...
void proximity(int index, std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool> isProcessed, KdTree* tree, float& distanceTol) {
	isProcessed[index] = true;
	cluster.push_back(index);
	std::vector<int> nearbyPoints = tree->search(points[index], distanceTol);
	for (int id = 0; id < nearbyPoints.size(); id++) {
		if (!isProcessed[id]) {
